...
export const createMcpServer = (workspace) => {
    const server = new McpServer({
        name: "polyrepo-mcp",
        version: "0.1.0"
    });
    server.tool("load_workspace", "Read .workspace.yaml and index repositories", {
        path: z.string().min(1)
    }, async ({ path }) => {
    ...
    server.tool("search_across_repos", "Search for text/regex across all or selected repos", {
    ...
    server.tool("find_usages", "Find symbol usages across repositories", {
    ...
    server.tool("get_file", "Get a file by repo and relative path", {
        repo: z.string().min(1),
        path: z.string().min(1)
    }, async ({ repo, path }) => {
    ...
    server.tool("diff_interfaces", "Compare same file across two repos for interface drift", {
